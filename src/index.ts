...
export default {
    async fetch(request, env) {
        ...
        // Caso 1: Mostrar la imagen guardada en KV.
        if (url.pathname === '/imagen' && imageId) {
            try {
                const imageBlob = await env._STATIC_CONTENT.get(imageId, 'arrayBuffer');
                if (imageBlob) {
                    return new Response(imageBlob, {
                        headers: {
                            'Content-Type': 'image/png',
                            'Cache-Control': 'public, max-age=3600', // Cachear la imagen por 1 hora
                        },
                    });
                } else {
                ...
                return new Response(`Error al obtener la imagen: ${error.message}`, { status: 500 });
            }
        }
        
        // Caso 2: Generar una nueva imagen, guardarla en KV y mostrar la página de resultados.
        if (url.pathname === '/resultado' && userPrompt) {
            ...
            try {
                ...
                // Generar una ID única y guardar la imagen en KV por 10 minutos
                const imageId = `${Date.now()}`;
                await env._STATIC_CONTENT.put(imageId, imageBlob, { expirationTtl: 600 });
...
<div class="image-container">
    <img src="/imagen?id=${imageId}" alt="Imagen generada por IA">
</div>
<div class="options">
    <a href="/imagen?id=${imageId}" download="imagen-ia.png" class="download-btn">Descargar imagen</a>
    <a href="/" class="new-image-btn">Generar otra imagen</a>
        ...
        // Caso 3: Mostrar el formulario inicial.
        if (url.pathname === '/') {
            const html =
                `<!DOCTYPE html>
<html>
